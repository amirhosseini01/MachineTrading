...
public class PlaywrightService(MachineTradingContext context)
{
    ...
    public async Task StartScrapping(string url, bool continueUntilPrevious = true, CancellationToken ct = default)
    {
        ...
        var previousArticles = await context.Articles.OrderByDescending(x => x.Time).Take(1000).ToListAsync(ct);
